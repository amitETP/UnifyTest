...
//		         String  = cell[i+5];
			...
			WebDriverWait waitx = new WebDriverWait(driver, Duration.ofSeconds(50));
			...
			WebElement element4 = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//button[contains(text(),' BASIC ')]")));
			element4.click();
...
//	   		}
			...
			int kk = 0;
			while (true) {
				if ((Integer.parseInt(dates1[2])) < (Integer.parseInt(driver.findElement(By.xpath("(//td[@role='gridcell'])[1]")).getText()))) {
					Thread.sleep(1000);
					driver.findElement(By.xpath("//div[@class='mat-calendar-controls']/button[2]")).click();
